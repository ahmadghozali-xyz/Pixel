...
(async() => {
    ...
    try {
        for (let i = 1; i <= jumlah; i++) {
            ...
            const domain = domains[randomIndex].trim(); // Trim spasi yang tidak diperlukan

            var name = random.first().replace(/\s/g, ''); // Menghapus spasi
            var lastname = random.last().replace(/\s/g, ''); // Menghapus spasi
            var uname = `${name}${lastname}${randomize('0', 2)}`;
            var email = `${uname}@${domain}`;

            // Regex untuk memastikan format email yang valid
            ...
            await new Promise(resolve => setTimeout(resolve, (Math.random() * 500) + 3000));      
        }
    } catch (error) {
        console.error('[Error]:', error);
    }
